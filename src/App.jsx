...
function addMonthsPreserveDay(dateStr, months) {
  const [y, m, day] = dateStr.split("-").map(Number);
  return toDateStr(new Date(y, m - 1 + months, day));
}
...
function computeNextRecurrenceDate(currentDateStr, weekday, shiftHoliday) {
  ...
  if (weekday === "3m") return addMonthsPreserveDay(currentDateStr, 3);
  if (weekday === "6m") return addMonthsPreserveDay(currentDateStr, 6);
  if (weekday === "1y") return addMonthsPreserveDay(currentDateStr, 12);
                                        ...
                                        <div style={styles.subBody} className="sub-body">
                                          ...
                                          <div style={styles.scheduleEditRow}>
                                            ...
                                            <label style={styles.scheduleEditField}>
                                              ...
                                              <select value={s.repeatWeekday ?? ""} onChange={(e) => updateSubtaskSchedule(p.id, t.id, s.id, "repeatWeekday", e.target.value === "" ? "" : (["weekday", "satsun", "daily", "3m", "6m", "1y"].includes(e.target.value) ? e.target.value : Number(e.target.value)))} style={styles.scheduleEditInput}>
                                                ...
                                                <option value="3m">3ヶ月毎</option>
                                                <option value="6m">半年毎</option>
                                                <option value="1y">1年毎</option>
                                              </select>
                                            </label>
...
const styles = {
  ...
  calGridWrap: { border: "1px solid #E0E0E0", borderRadius: 6, overflowX: "auto", overflowY: "hidden", maxWidth: "100%" },
  calGridHeaderRow: { display: "flex", borderBottom: "1px solid #E0E0E0", background: "#F9F9F9", width: "fit-content", minWidth: "100%" },
  calGridTimeColHeader: { width: 40, flexShrink: 0 },
  calGridDayHeaderCol: { flex: "1 0 90px", minWidth: 90, display: "flex", flexDirection: "column", gap: 2, padding: "4px 3px", borderLeft: "1px solid #E0E0E0" },
  ...
  calGridBody: { display: "flex", position: "relative", overflowY: "auto", maxHeight: 480, width: "fit-content", minWidth: "100%" },
